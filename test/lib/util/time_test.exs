...
defmodule Antikythera.TimeTest do
  ...
  test "from_iso_timestamp should try to convert string to time" do
    ...
    assert Time.from_iso_timestamp("2016-02-24T19:55:23.974+09:00")        == Time.from_iso_timestamp("2016-02-24T10:55:23.974+00:00")
    assert Time.from_iso_timestamp("2016-02-24T19:55:23.974+09:00")        == Time.from_iso_timestamp("2016-02-24T10:55:23.974Z")
    assert Time.from_iso_timestamp("2016-02-24T19:55:23.000+09:00")        == Time.from_iso_timestamp("2016-02-24T10:55:23+00:00")
    assert Time.from_iso_timestamp("2016-02-24T19:55:23.000+0900" )        == Time.from_iso_timestamp("2016-02-24T10:55:23+00:00")
    assert Time.from_iso_timestamp("2016-02-24T19:55:23.000+09:00")        == Time.from_iso_timestamp("2016-02-24T10:55:23Z")
  ...
  test "from_http_date should try to convert HTTP date string to time" do
    ...
    assert Time.from_http_date("Thursday, 25-Feb-16 00:00:00 GMT") |> R.get!() == t2 # RFC850 format
    assert Time.from_http_date("Thu Feb 25 00:00:00 2016"        ) |> R.get!() == t2 # ANSI C's asctime() format
    assert Time.from_http_date("Thu, 25 Feb 2016 00:00:00 JST"   ) |> R.get!() == t2 # Timezone string just ignored
  ...
  test "should be converted to JSON using Poison.encode/1" do
    assert Poison.encode({Time, {2017, 1, 1}, {0, 0, 0}, 0}) == {:ok, ~S("2017-01-01T00:00:00.000+00:00")}
    assert Poison.encode({Time, {2017, 1, 0}, {0, 0, 0}, 0}) == {:error, {:invalid, {Time, {2017, 1, 0}, {0, 0, 0}, 0}}}

    map_with_time = %{"time" => {Time, {2017, 1, 1}, {0, 0, 0}, 0}}
    assert Poison.encode( map_with_time)                                                    == {:ok, ~S({"time":"2017-01-01T00:00:00.000+00:00"})}
    assert Poison.encode!(map_with_time) |> Poison.decode!() |> S.new!() |> Poison.encode() == {:ok, ~S({"time":"2017-01-01T00:00:00.000+00:00"})}
  end
end
