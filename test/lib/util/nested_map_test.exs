...
defmodule Antikythera.NestedMapTest do
  ...
  test "deep_merge/2" do
    ...
    assert M.deep_merge(%{a: %{b: %{x: 1, y: 2}, c: %{z: 3}}}, %{a: %{b: %{y: 3, z: 4}}}) == %{a: %{b: %{x: 1, y: 3, z: 4}, c: %{z: 3}}}
  end
  ...
  test "update_existing_in/3" do
    ...
    assert M.update_existing_in(%{a: %{b: %{c: 1}}}, [:a, :b, :c], f) == {:ok, %{a: %{b: %{c: 2}}}}
    ...
    catch_error M.update_existing_in(%{a: %{b: "non_int"}}, [:a, :b], f) # callback error should be thrown out
  end
end
