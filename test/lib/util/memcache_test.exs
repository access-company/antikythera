...
defmodule Antikythera.MemcacheTest do
  ...
  test "should write values to memcache and read them with read_or_else_write/5" do
    ...
    assert Memcache.read_or_else_write("foo", @epool_id, @lifetime, fn -> "bar" end) ==
             {:ok, "bar"}
  ...
  test "should return an error with too large keys" do
    ...
    assert Memcache.read_or_else_write(large_binary, @epool_id, @lifetime, fn -> "bar" end) ==
             {:error, :too_large_key}
  end
  ...
  test "should return an error with too large values" do
    ...
    assert Memcache.read_or_else_write("foo", @epool_id, @lifetime, fn -> large_binary end) ==
