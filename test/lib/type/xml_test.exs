...
<note>
  ...
  test "should decode and encode XML while keeping its information through JSON serialization" do
    ...
    assert note_xml |> Poison.encode!() |> Poison.decode!() |> E.new!() |> Xml.encode(with_header: true) == @note_xml
    trimmed_note_xml = Xml.decode!(@note_xml, trim: true)
    assert trimmed_note_xml |> Xml.encode(pretty: true, with_header: true) == @note_xml
    assert trimmed_note_xml |> Poison.encode!() |> Poison.decode!() |> E.new!() |> Xml.encode(pretty: true, with_header: true) == @note_xml
  end
  ...
  test "should accept Access lookups/updates" do
    ...
    assert get_in(books_xml, ["book", Access.at(0)]) ==
      %E{name: "book", attributes: %{"id" => "bk101"}, children:
        [
          ...
          %E{name: "description", attributes: %{}, children: ["An in-depth look at creating applications \n      with XML."]},
        ]
      }
    assert get_in(books_xml, ["book", Access.all(), "genre", Access.at(0), :texts, Access.at(0)]) |> Enum.uniq() == ["Computer", "Fantasy", "Romance", "Horror", "Science Fiction"]
    {get_value, updated_xml} = get_and_update_in(books_xml, [:children, Access.all(), :children, Access.at(2), :children, Access.at(0)], &{&1, String.upcase(&1)})
    assert Enum.uniq(get_value) == ["Computer", "Fantasy", "Romance", "Horror", "Science Fiction"]
    assert get_in(updated_xml, ["book", Access.all(), "genre", Access.at(0), :texts, Access.at(0)]) |> Enum.uniq() == ["COMPUTER", "FANTASY", "ROMANCE", "HORROR", "SCIENCE FICTION"]
  end

  test "should handle attributes with nested quotations" do
    assert Xml.decode!(~S{<a name="outer'inner'"/>}) == %E{name: "a", attributes: %{"name" => ~S{outer'inner'}}, children: []}
    assert Xml.decode!(~S{<a name='outer"inner"'/>}) == %E{name: "a", attributes: %{"name" => ~S{outer"inner"}}, children: []}

    # Escape on encode
    assert Xml.decode!(~S{<a name="outer'inner'"/>}) |> Xml.encode() == ~S{<a name='outer&apos;inner&apos;'/>}
    assert Xml.decode!(~S{<a name='outer"inner"'/>}) |> Xml.encode() == ~S{<a name='outer&quot;inner&quot;'/>}
  end
end
