defmodule Mix.Tasks.Compile.GearStaticAnalysisTest do
  ...
  @tmp_ex_file_path Path.join(__DIR__, "tmp_ex_file_for_gear_static_analysis_test.ex")
  ...
  setup do
    ...
    on_exit(fn ->
      ...
      File.rm!(@tmp_ex_file_path)
    end)
  end

  # Gear name is `antikythera` in the following tests
  describe "find_issues_in_file/1" do
    test "should detect naming module which is not prefixed with the gear name" do
      File.write!(@tmp_ex_file_path, """
      defmodule Foo do
        def bar(), do: :buz
      end
      """)
      ...
      assert [
               {:error, @tmp_ex_file_path, _meta,
                "module name `Foo` is not prefixed with the gear name"}
             ] = GearStaticAnalysis.find_issues_in_file(@tmp_ex_file_path)
    end

    test "should detect implementing non-gear-specific protocol for non-gear-specific type" do
      File.write!(@tmp_ex_file_path, """
    ...
    test "should detect directly using Gettext" do
      File.write!(@tmp_ex_file_path, """
    ...
    test "should detect disturbing execution of ErlangVM" do
      File.write!(@tmp_ex_file_path, """
      defmodule Antikythera.Dummy do
    ...
    test "should detect writing to STDOUT/STDERR" do
      File.write!(@tmp_ex_file_path, """
      defmodule Antikythera.Dummy do
    ...
    test "should detect spawning processes" do
      ...
      assert [
               {:error, @tmp_ex_file_path, _meta,
                "spawning processes in gear's code is prohibited"}
             ] = GearStaticAnalysis.find_issues_in_file(@tmp_ex_file_path)
    end

    test "should detect calling :os.cmd/1" do
      File.write!(@tmp_ex_file_path, """
      defmodule Antikythera.Dummy do
      ...
      assert [
               {:error, @tmp_ex_file_path, _meta,
                "calling :os.cmd/1 in gear's code is prohibited"}
             ] = GearStaticAnalysis.find_issues_in_file(@tmp_ex_file_path)
    end

    test "should detect calling System.cmd/3" do
      File.write!(@tmp_ex_file_path, """
      defmodule Antikythera.Dummy do
      ...
      assert [
               {:error, @tmp_ex_file_path, _meta,
                "calling System.cmd/3 in gear's code is prohibited"}
             ] = GearStaticAnalysis.find_issues_in_file(@tmp_ex_file_path)
    end

    test "should detect spawning processes by local call" do
      File.write!(@tmp_ex_file_path, """
      defmodule Antikythera.Dummy do
    ...
    test "should detect using AntikytheraCore.*" do
      File.write!(@tmp_ex_file_path, """
      defmodule Antikythera.Dummy do
      ...
      assert [
               {:error, @tmp_ex_file_path, _meta,
                "direct use of `AntikytheraCore.*` is prohibited"}
             ] = GearStaticAnalysis.find_issues_in_file(@tmp_ex_file_path)
    end

    test "should detect using AntikytheraEal.*" do
      File.write!(@tmp_ex_file_path, """
      defmodule Antikythera.Dummy do
      ...
      assert [
               {:error, @tmp_ex_file_path, _meta,
                "direct use of `AntikytheraEal.*` is prohibited"}
             ] = GearStaticAnalysis.find_issues_in_file(@tmp_ex_file_path)
    end

    test "should detect using AntikytheraLocal.*" do
      File.write!(@tmp_ex_file_path, """
      defmodule Antikythera.Dummy do
      ...
      assert [
               {:error, @tmp_ex_file_path, _meta,
                "direct use of `AntikytheraLocal.*` is prohibited"}
             ] = GearStaticAnalysis.find_issues_in_file(@tmp_ex_file_path)
    end

    test "should detect using Antikythera.Test.*" do
      File.write!(@tmp_ex_file_path, """
      defmodule Antikythera.Dummy do
    ...
    test "should detect using Antikythera.Mix.Task.*" do
      File.write!(@tmp_ex_file_path, """
      defmodule Antikythera.Dummy do
    ...
    test "should detect directly using :hackney" do
      File.write!(@tmp_ex_file_path, """
      defmodule Antikythera.Dummy do
