...
defmodule AntikytheraCore.Handler.WebsocketState do
  ...
  defun make(conn :: v[Conn.t], {ws_module, _action} :: GearEntryPoint.t, helper_modules :: v[HelperModules.t]) :: t do
    %__MODULE__{
      conn:            conn,
      ws_module:       ws_module,
      gear_impl_state: nil,
  ...
  @type cowboy_ws_terminate_reason :: :normal | :stop | :timeout
  ...
  defun handle_client_message(%__MODULE__{conn: conn, ws_module: ws_module, gear_impl_state: gear_impl_state} = state,
                              frame :: v[Frame.t]) :: callback_result do
    run_callback_and_reply(state, 1, fn -> ws_module.handle_client_message(gear_impl_state, conn, frame) end)
  end

  defun handle_server_message(%__MODULE__{conn: conn, ws_module: ws_module, gear_impl_state: gear_impl_state} = state,
                              message :: any) :: callback_result do
    run_callback_and_reply(state, 0, fn -> ws_module.handle_server_message(gear_impl_state, conn, message) end)
  end

  defunp run_callback_and_reply(state :: v[t], n_received :: v[non_neg_integer], f :: (() -> callback_result)) :: callback_result do
    try do
      {:ok, f.()}
    catch
    ...
    |> case do
      {:ok, {new_gear_impl_state, frames_to_send}} ->
        ...
        new_state = increment_frames_count(%__MODULE__{state | gear_impl_state: new_gear_impl_state}, n_received, n_sent)
        case n_sent do
          0 -> {:ok, new_state}
  ...
  defun terminate(%__MODULE__{conn: conn, ws_module: ws_module, gear_impl_state: gear_impl_state} = state,
    ...
    try do
      ...
      :error, error  -> log_error(state, now, ErrorReason.format({:error, error }, System.stacktrace()))
      :throw, value  -> log_error(state, now, ErrorReason.format({:throw, value }, System.stacktrace()))
      :exit , reason -> log_error(state, now, ErrorReason.format({:error, reason}, System.stacktrace()))
    end
  end

  defunp terminate_reason(%__MODULE__{error_reason: error_reason},
                          cowboy_terminate_reason :: cowboy_ws_terminate_reason) :: Websocket.terminate_reason do
  ...
  for level <- [:info, :error] do
    defunp unquote(:"log_#{level}")(%__MODULE__{conn: %Conn{context: %Context{context_id: context_id}},
                                                helper_modules: %HelperModules{logger: logger}},
                                    time    :: v[Time.t],
                                    message :: v[String.t]) :: :ok do
      Writer.unquote(level)(logger, time, context_id, "<websocket> " <> message)
    end
  end

  defunp increment_frames_count(%__MODULE__{frames_received: frames_received, frames_sent: frames_sent} = state,
                                n_received :: v[non_neg_integer],
                                n_sent     :: v[non_neg_integer]) :: t do
    submit_metrics_if_any(state, n_received, n_sent)
    %__MODULE__{state | frames_received: frames_received + n_received, frames_sent: frames_sent + n_sent}
  end

  defunp submit_metrics_if_any(%__MODULE__{conn:           %Conn{context: %Context{executor_pool_id: epool_id}},
                                           helper_modules: %HelperModules{metrics_uploader: uploader}},
                               n_received :: v[non_neg_integer],
                               n_sent     :: v[non_neg_integer]) :: :ok do
  ...
  defp build_metrics(n_r, n_s), do: [{"websocket_frames_received", :sum, n_r}, {"websocket_frames_sent", :sum, n_s}]
end
