...
defmodule AntikytheraCore.Metrics.Buffer do
  ...
  defun add(buffer :: t, now :: v[Time.t], list :: [{String.t, Strategy.t, number}], epool_id :: epool_id) :: t do
    unit = {Time.truncate_to_minute(now), epool_id}
    new_data_map = Enum.reduce(list, buffer[unit] || %{}, fn({type, strategy, value}, map) ->
      key = {type, strategy}
  ...
  defun partition_ongoing_and_past(buffer :: t, now :: v[Time.t]) :: {t, [{metrics_unit, metrics_data_map}]} do
    now_minute = Time.truncate_to_minute(now)
    {ongoing, past} = Enum.split_with(buffer, fn {{minute, _epool_id}, _data_map} ->
