...
defmodule AntikytheraCore.Path do
    ...
    defun compiled_core_dir()    :: Path.t do
      ...
      release_generating_project_dir =
        case Path.basename(parent_dir) do
          "deps" -> Path.join([parent_dir, ".."])    # from antikythera instance or gear projects
  ...
  defun tenant_setting_file_path(id :: v[TenantId.t]) :: Path.t, do: Path.join(tenant_setting_dir(), id)
  ...
  defunp modified_regular_file?(path :: Path.t, since :: v[SecondsSinceEpoch.t]) :: boolean do
    case File.stat(path, [time: :posix]) do
      ...
      {:error, :enoent}                                           -> false # removed between `Path.wildcard/1` and `File.stat/2`
    end
  end
end
