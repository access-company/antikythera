...
defmodule AntikytheraCore.Version.Core do
  ...
  defunp current_antikythera_instance_version!() :: VersionStr.t do
    Version.current_version(Env.antikythera_instance_name()) || raise "antikythera instance version is not found!"
  end
  ...
  defun upgrade_to_next_version() :: :ok do
    ...
    case History.next_upgradable_version(Env.antikythera_instance_name(), current_version) do
      nil          -> L.info("skip upgrade of antikythera instance: already the latest (#{current_version})")
  ...
  defunp upgrade(version :: v[VersionStr.t]) :: :ok do
    ...
    with {:ok, version_charlist} <- :release_handler.unpack_release('#{version}/#{Env.antikythera_instance_name()}'),
  ...
  defunp emit_success_message_to_all_log_files(version :: v[VersionStr.t]) :: :ok do
    ...
    L.info(message)
    GearManager.running_gear_names() |> Enum.each(fn gear_name ->
