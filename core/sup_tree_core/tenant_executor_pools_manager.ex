...
defmodule AntikytheraCore.TenantExecutorPoolsManager do
  ...
  def handle_info(:check, %{last_checked_at: last_checked_at, fetcher: fetcher} = s) do
    ...
    case fetcher do
      ...
      # don't run more than one temporary process
      {_pid, _ref} -> {:noreply, s}
    end
  end
  ...
  def handle_info({:DOWN, _monitor_ref, :process, _pid, reason}, s0) do
    ...
    s2 =
      case reason do
        {:ok, checked_at, result} ->
          result_received(s1, checked_at, result)
  ...
  defp result_received(
         %{settings: old_settings, to_remove: to_remove, to_disassociate: to_disassociate} = s,
         checked_at,
         {result_set_type, modified_settings}
       ) do
    new_settings1 =
      apply_pending_operations(
        ...
        to_remove,
        to_disassociate,
        Map.keys(modified_settings)
      )

    {new_settings2, new_to_remove, new_to_disassociate} =
      apply_modifications(new_settings1, result_set_type, modified_settings)

    %{
      s
      | last_checked_at: checked_at,
        settings: new_settings2,
        to_remove: new_to_remove,
        to_disassociate: new_to_disassociate
    }
  end
  ...
  defunp handle_updated_tenant_settings(
    ...
    m =
      MapUtil.map_values(modified, fn {tenant_id, {old, new}} ->
        case {old.gears, new.gears} do
          ...
          {[], _} ->
            start_executor_pool(tenant_id, new)
            {new, []}
  ...
  defunp start_executor_pool(tenant_id :: v[TenantId.t()], tsetting :: v[TenantSetting.t()]) ::
           :ok do
  ...
  defunp update_existing_executor_pool(
           tenant_id :: v[TenantId.t()],
           old_tsetting :: v[TenantSetting.t()],
           new_tsetting :: v[TenantSetting.t()]
         ) :: :ok do
  ...
  defun all() :: settings do
    # simply use `:sys.get_state/1` and skip defining internal message
    %{settings: settings} = :sys.get_state(__MODULE__)
    settings
  end
end
