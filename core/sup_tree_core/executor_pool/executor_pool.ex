...
defmodule AntikytheraCore.ExecutorPool do
  ...
  @wait_time_for_broker_termination if Mix.env() == :test,
                                      do: 100,
                                      else: TenantExecutorPoolsManager.polling_interval() * 2
  ...
  defp action_runner_pool_multi_args(epool_id, n_pools_a, size_a) do
    [
      ...
      epool_id,
      size_a,
      # we don't use ondemand worker processes for gear actions
      0,
      [
  ...
  defp async_job_runner_pool_args(epool_id, size_j, job_pool_name) do
    [
      AsyncJobRunner,
      epool_id,
      # we don't use reserved worker processes for async jobs
      0,
      size_j,
      [
        name: job_pool_name,
        # must be longer than async job maximum duration (30m)
        checkout_max_duration: 2400
      ]
    ]
  end
