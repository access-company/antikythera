...
defmodule Antikythera.Mixfile do
  ...
  defp deps() do
    ...
    [
      ...
      # relx
      {:bbmustache, "1.0.4", [indirect: true, only: :prod, runtime: false]},
      # relx
      {:cf, "0.2.2", [indirect: true, only: :prod, runtime: false]},
      # relx
      {:erlware_commons, "1.0.0", [indirect: true, only: :prod, runtime: false]},
      # relx
      {:getopt, "0.8.2", [indirect: true, only: :prod, runtime: false]},
      # relx
      {:providers, "1.6.0", [indirect: true, only: :prod, runtime: false]},

      # indirect tool deps
      # credo
      {:bunt, "0.2.0", [indirect: true, only: :dev]},
      # ex_doc
      {:earmark, "1.4.2", [indirect: true, only: :dev]},
      # ex_doc
      {:makeup_elixir, "0.14.0", [indirect: true, only: :dev]},
      # makeup_elixir
      {:makeup, "1.0.0", [indirect: true, only: :dev]},
      # makeup
      {:nimble_parsec, "0.5.2", [indirect: true, only: :dev]},
      # exsync and mix_test_watch
      {:file_system, "0.2.7", [indirect: true, only: :dev]}
    ]
  end
