...
defmodule Mix.Tasks.Antikythera.Gear.New do
  ...
  @symlink_candidates [
    {".credo.exs", "Configuration file for static code analysis tool [Credo](https://github.com/rrrene/credo)"},
    {".tool-versions", "Local versions file for version manager tool [asdf](https://github.com/asdf-vm/asdf)"},
  ]
  ...
  def run(args) do
    case args do
      ...
      _ -> Mix.raise("Usage: mix antikythera.gear.new PATH")
    end
  end
  ...
  defp instance_git_url_from_git_remote!() do
    case System.cmd("git", ["remote", "get-url", "origin"]) do
      {instance_git_url, 0       } -> String.trim(instance_git_url)
      {output          , _nonzero} -> Mix.raise("Tried to read url of git remote 'origin' for antikythera instance dependency, but encountered the following error:\n#{output}")
    end
  end
