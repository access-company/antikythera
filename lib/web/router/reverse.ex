...
defmodule SolomonLib.Router.Reverse do
  ...
  defun make_path(path_pattern :: v[String.t], fillers :: v[[placeholder_filler]], query_params :: v[%{String.t => String.t}]) :: String.t do
    replaced_path = fill_path(String.split(path_pattern, "/"), fillers, [])
    query = Enum.map_join(query_params, "&", fn
  ...
  defunp fill_path(patterns :: [String.t], fillers :: [placeholder_filler], acc :: [String.t]) :: String.t do
    ([]                   , []                , acc) -> Enum.reverse(acc) |> Enum.join("/")
    ([":" <> _ | patterns], [filler | fillers], acc) -> fill_path(patterns, fillers, [encode_segment(filler) | acc])
    (["*" <> _ | patterns], [filler | fillers], acc) -> fill_path(patterns, fillers, [encode_segment_list(filler) | acc])
    ([fixed | patterns]   , fillers           , acc) -> fill_path(patterns, fillers, [fixed | acc])
  end
