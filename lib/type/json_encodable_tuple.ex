...
# implements tuple-to-string conversion for specific types (especially SolomonLib.Time.t).
...
defimpl Poison.Encoder, for: Tuple do
  ...
  def encode(                         tuple, _options) do
    raise Poison.EncodeError, value: tuple, message: "cannot encode unsupported tuple, got: #{inspect(tuple)}"
  end
  ...
  defp encode_time(time, _options) do
    if Time.valid?(time) do
      ...
      raise Poison.EncodeError, value: time, message: "invalid SolomonLib.Time.t, got: #{inspect(time)}"
    end
  end
end
