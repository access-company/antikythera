...
defmodule Antikythera.FastJasonEncoder do
  ...
  defun encode(value :: any, opts :: Jason.Encode.opts) :: iodata do
    ...
    (%DateTime{} = value, _   )                       -> [?", DateTime.to_iso8601(value), ?"]
  ...
  defunp encode_map(value :: [tuple], opts :: Jason.Encode.opts, comma :: boolean) :: iodata do
    ([]                        , _   , _    ) -> '}'
    ([{:__struct__, _v} | tail], opts, comma) -> encode_map(tail, opts, comma)
    (list                      , opts, true ) -> [?,, encode_map(list, opts, false)]
    ([{k,            v} | tail], opts, false) -> [encode(k, opts), ?:, encode(v, opts) | encode_map(tail, opts, true)]
  end
