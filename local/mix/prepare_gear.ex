...
defmodule Mix.Tasks.AntikytheraLocal.PrepareGear do
  ...
  def run(args) do
    ...
    version =
      AntikytheraLocal.RunningEnvironment.prepare_new_version_of_gear(gear_repo_dir, do_upgrade)

    output_version = if do_upgrade, do: version, else: "#{version} noupgrade"

    IO.puts(
      "Successfully prepared new version of #{Path.basename(gear_repo_dir)} (#{output_version})"
    )
  end
end
